...
def _process_occlude_image(
    ...
    while top_left_y < image.shape[0] - patch_size or top_left_x < image.shape[1]-patch_size:
        # TODO: Replace this with a simpler ravel expression
...
def GRADCAM(
    ...
    subplot_titles = []
    if titles:
        for i, (labels, prediction) in enumerate(zip(labels, predictions)):
            subplot_titles.append(f"True {i}\nPred {prediction} ({tf.nn.softmax(prediction)[i]:.2f})")
